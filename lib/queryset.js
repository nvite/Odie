...
/**
 * Iterates over a QuerySet, formatting each instance with the supplied formatOptions
 * @param  {Object}   formatOptions The options to supply to each instance's `format` method
 * @param  {Function} callback      The callback to run when finished
 * @return {Object}                 A formatted Object representation of the queryset
 */
...
/**
 * Iterates a cursor, calling `next` on each result until the cursor is exhausted
 * @param  {Function} callback The iterator function to perform with the model instance
 * @param  {Function} done     The final callback to run when the cursor is exhausted
 * @return {Void}            [description]
 */
QuerySet.prototype.whileNext = function (callback, done) {
  this.next(function (err, instance) {
    if (err) {
      return done.call(null, err);
    }
    if (instance === null) {
      return done.call(null);
    }
    callback.call(null, instance);
    return this.whileNext(callback, done);
  }.bind(this));
};
