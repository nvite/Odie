...
module.exports = function initializeAccessors (Klass) {
  ...
  /**
   * Sets a property or array of properties as observable to the given context
   * @param  {String} context      An optional string name of the context in which the propert(ies) are observable
   * @param  {Mixed}  properties   A string or array of strings representing the property names that are observable
   * @return {Void}
   */
  Klass.observable = function (context, properties) {
    if (arguments.length === 1) {
      properties = context;
      context = 'default';
    }
    var refArr = utils.getPathReferenceArray(Klass, 'OBSERVABLE_PROPERTIES.' + context, {create: true});
    refArr[0][refArr[1]] || (refArr[0][refArr[1]] = []);
    // we use a Set to dedupe properties. We also concat _id on each set to make sure the _id is always observable.
  ...
  /**
   * A shorthand for setting `observable` and `writable` in one call
   * @param  {Mixed}  properties
   * @param  {String} context
   * @return {Void}
   */
  Klass.accessible = function (properties, context) {
    Klass.observable.apply(null, Array.prototype.slice.call(arguments));
